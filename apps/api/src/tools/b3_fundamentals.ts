...
export function b3FundamentalsTool(source: Source, log: Logger): Tool {
  return {
    name: 'b3_fundamentals',
    description: 'Retorna indicadores fundamentalistas (P/L, P/VP, DY, ROE, etc.) de um ativo listado na B3.',
    schema: {
      type: 'object',
      properties: {
    ...
    handler: async (args, signal) => {
      ...
      const t0 = Date.now();
      try {
        const f = await source.fundamentals(ticker, signal);
        log.debug({ ticker, durationMs: Date.now() - t0 }, 'b3_fundamentals executed');
        return f;
      } catch (err) {
